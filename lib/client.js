window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function storeImage(images, dataUrl) {
			...
			let key = base;
			let suffix = 1;
			while (images[key] !== void 0 && images[key] !== dataUrl) {
				key = base + "-" + suffix;
				suffix += 1;
			}
			images[key] = dataUrl;
		...
		function compactImages(tree) {
			...
			visit(tree.items || []);
			const images = {};
			for (const ref of refs) if (tree.images && typeof tree.images[ref] === "string") images[ref] = tree.images[ref];
			tree.images = images;
		}
...
`;
		...
		function apply(ctx) {
			...
			if (slots === void 0 || layout === void 0 || connection === void 0) throw new Error("dsh-outline-plugin: slots, layout, and connection are required");
			ctx.effect(insertStyles, "dsh-outline-plugin: styles");
			const host = { async call(method, payload) {
			...
			function ItemRow(props) {
				...
				react.useEffect(() => {
					...
					if (el && el.tagName === "TEXTAREA" && typeof el.setSelectionRange === "function") try {
				...
				function onEditKeyDown(e) {
					...
					if (e.key === "Enter" && !e.shiftKey) {
						...
						setEditing(false);
						onMutate();
					} else if (e.key === "Enter" && e.shiftKey) try {
						const t = e.target;
						t.style.height = "auto";
						...
						e.preventDefault();
						setText(tree, node.id, editText);
						if (e.shiftKey ? outdent(tree, node.id) : indent(tree, node.id)) onMutate();
					...
					for (let i = 0; i < items.length; i += 1) {
						...
						if (it.kind === "file" && it.type && it.type.indexOf("image/") === 0) {
							...
							reader.onload = () => {
								...
								if (!url || url.length > 8e6) return;
								downscaleImage(url, 1600, .85).then((small) => {
				...
				const row = react.createElement("div", {
					...
					node.kind === "todo" ? react.createElement("input", {
						key: "check",
						type: "checkbox",
						...
						key: "bullet",
						className: "dsh-outline-bullet",
						draggable: false
					}),
					react.createElement("div", {
						key: "content",
						className: "dsh-outline-content"
					...
					}, node.text), src ? react.createElement("div", {
						key: "imgwrap",
						className: "dsh-outline-imgwrap"
					}, [react.createElement("img", {
						key: "img",
						className: "dsh-outline-img",
						src,
						alt: ""
					}), react.createElement("button", {
						key: "rm",
				...
				const childRows = node.collapsed || !hasChildren ? [] : children.map((child) => react.createElement(ItemRow, {
					key: child.id,
					node: child,
					depth: depth + 1,
					tree,
			...
			function Panel(props) {
				...
				const rows = filterItems(tree.items, view).map((n) => react.createElement(ItemRow, {
					key: n.id,
					node: n,
					depth: 0,
					tree,
				...
				let body;
				if (status === "loading") body = react.createElement("div", { className: "dsh-outline-hint" }, "加载中…");
				else if (status === "error") body = react.createElement("div", { className: "dsh-outline-error" }, "加载失败，请收起后重新打开面板");
				else if (view === "attach") {
					const gallery = collectImages(tree.items, []).map((n) => {
						const s = imageUrl(tree, n);
						if (!s) return null;
						return react.createElement("div", {
							key: n.id,
							className: "dsh-outline-attach"
						}, [
							react.createElement("img", {
								key: "img",
								className: "dsh-outline-attach-img",
								src: s,
								alt: ""
							}),
							react.createElement("div", {
								key: "cap",
								className: "dsh-outline-attach-cap"
							}, n.text || "（无标题）"),
							react.createElement("button", {
								key: "rm",
					...
					body = gallery.length > 0 ? react.createElement("div", { className: "dsh-outline-attach-grid" }, gallery) : react.createElement("div", { className: "dsh-outline-hint" }, "暂无图片：在笔记中粘贴图片后会显示在这里");
				} else if (rows.length === 0) {
					const addButtons = [];
					if (view === "all" || view === "todo") addButtons.push(react.createElement("button", {
						key: "todo",
						className: "dsh-outline-add",
						onClick: () => addTop("todo")
					}, "＋待办"));
					if (view === "all" || view === "note") addButtons.push(react.createElement("button", {
						key: "note",
						className: "dsh-outline-add",
						onClick: () => addTop("note")
					}, "＋笔记"));
					body = react.createElement("div", null, [react.createElement("div", {
						key: "hint",
						className: "dsh-outline-hint"
					}, "暂无内容，添加一条开始（单击条目即可编辑，Enter 新建同级，编辑时可直接粘贴图片）"), react.createElement("div", {
						key: "acts",
						className: "dsh-outline-empty-actions"
					}, addButtons)]);
				} else body = rows;
				let countText = "";
				...
				return react.createElement("div", {
					...
					react.createElement("div", {
						...
						react.createElement("span", {
							key: "counts",
							className: "dsh-outline-progress"
						}, countText),
						react.createElement("button", {
							key: "f-",
							className: "dsh-outline-add",
					...
					showSettings ? react.createElement("div", { key: "settings" }, [react.createElement("div", {
						key: "row",
						className: "dsh-outline-settings"
					}, [
						react.createElement("span", {
							key: "label",
							style: {
						...
						key: "hint",
						className: "dsh-outline-settings-hint"
					}, pathMsg || (mdPath ? "当前导出：" + mdPath : "数据存储于宿主工作区 .dsh-outline.json（非浏览器 localStorage）"))]) : null,
					snap.saveError ? react.createElement("div", {
						key: "save-error",
						className: "dsh-outline-error",
						role: "status"
					}, "保存失败：" + snap.saveError) : null,
					react.createElement("div", {
						key: "body",
						className: "dsh-outline-body"
					}, body)
				]);
			}
